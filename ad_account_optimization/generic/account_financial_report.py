...
class report_account_common(report_sxw.rml_parse, common_report_header):
    #print "xxxxxxxx"
    _name = 'report.account.financial.report.mirror'
...
#        return data['form']['chart_account_id'][1]
    ...
    def get_lines_another(self, group):
        print "qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq", group
        self.pool.get('account.move.line').search(self.cr, self.uid, {'report_type','=','asset'})
        print "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA", self.result.get(group, [])
        #print "self.result.get(group, [])------------------------->>", self.result.get(group, [])
        return self.result.get(group, [])
    
report_sxw.report_sxw('report.account.financial.report.mirror', 'account.financial.report',
    'addons/ad_account_optimization/report/account_financial_report.rml', parser=report_account_common, header='internal')
