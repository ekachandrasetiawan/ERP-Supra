...
#         'type': fields.selection([('plan', 'Plan')], 'Type'),
	...
	def create_activity(self, cr, uid, ids, context=None):    
		...
		if val.beforeplansenin:
			plan_rels = self.update_activity(cr, uid, ids, actid, val.beforeplansenin, 'before.plan.senin')
			self.update_activity(cr, uid, ids, actid, val.beforeplansenin, 'before.actual.senin', 'actual',plan_rels)
		else:
			raise osv.except_osv(('Attention !'), ("Monday must have plan !"))
		if val.afterplansenin:
			plan_rels = self.update_activity(cr, uid, ids, actid, val.afterplansenin, 'after.plan.senin')
			self.update_activity(cr, uid, ids, actid, val.afterplansenin, 'after.actual.senin', 'actual',plan_rels)
		else:
			raise osv.except_osv(('Attention !'), ("Monday must have plan !"))
		if val.beforeplanselasa:
			plan_rels = self.update_activity(cr, uid, ids, actid, val.beforeplanselasa, 'before.plan.selasa')
			self.update_activity(cr, uid, ids, actid, val.beforeplanselasa, 'before.actual.selasa', 'actual',plan_rels)
		else:
			raise osv.except_osv(('Attention !'), ("Tuesday must have plan !"))
		if val.afterplanselasa:
			plan_rels = self.update_activity(cr, uid, ids, actid, val.afterplanselasa, 'after.plan.selasa')
			self.update_activity(cr, uid, ids, actid, val.afterplanselasa, 'after.actual.selasa', 'actual',plan_rels)
		else:
			raise osv.except_osv(('Attention !'), ("Tuesday must have plan !"))
		if val.beforeplanrabu:
			plan_rels = self.update_activity(cr, uid, ids, actid, val.beforeplanrabu, 'before.plan.rabu')
			self.update_activity(cr, uid, ids, actid, val.beforeplanrabu, 'before.actual.rabu', 'actual',plan_rels)
		else:
			raise osv.except_osv(('Attention !'), ("Wednesday must have plan !"))
		if val.afterplanrabu:
			plan_rels = self.update_activity(cr, uid, ids, actid, val.afterplanrabu, 'after.plan.rabu')
			self.update_activity(cr, uid, ids, actid, val.afterplanrabu, 'after.actual.rabu', 'actual',plan_rels)
		else:
			raise osv.except_osv(('Attention !'), ("Wednesday must have plan !"))
		if val.beforeplankamis:
			plan_rels = self.update_activity(cr, uid, ids, actid, val.beforeplankamis, 'before.plan.kamis')
			self.update_activity(cr, uid, ids, actid, val.beforeplankamis, 'before.actual.kamis', 'actual',plan_rels)
		else:
			raise osv.except_osv(('Attention !'), ("Thursday must have plan !"))
		if val.afterplankamis:
			plan_rels = self.update_activity(cr, uid, ids, actid, val.afterplankamis, 'after.plan.kamis')
			self.update_activity(cr, uid, ids, actid, val.afterplankamis, 'after.actual.kamis', 'actual',plan_rels)
		else:
			raise osv.except_osv(('Attention !'), ("Thursday must have plan !"))
		if val.beforeplanjumat:
			plan_rels = self.update_activity(cr, uid, ids, actid, val.beforeplanjumat, 'before.plan.jumat')
			self.update_activity(cr, uid, ids, actid, val.beforeplanjumat, 'before.actual.jumat', 'actual',plan_rels)
		else:
			raise osv.except_osv(('Attention !'), ("Friday must have plan !"))
		if val.afterplanjumat:
			plan_rels = self.update_activity(cr, uid, ids, actid, val.afterplanjumat, 'after.plan.jumat')
			self.update_activity(cr, uid, ids, actid, val.afterplanjumat, 'after.actual.jumat', 'actual',plan_rels)
		else:
			raise osv.except_osv(('Attention !'), ("Friday must have plan !"))
		# raise osv.except_osv(('Attention !'), ("ZZZZZZZZZZZZZZZZZZZZZZ !"))
			
	def update_activity(self, cr, uid, ids, actid, data, tabel, jenis='',plan_rels=[]):
		plan_ids = []
		a=0;
		for x in data:
			if jenis:
				# ACTUAL
				# print "RELSSSSS  ",plan_rels[a]
				self.pool.get(tabel).create(cr, uid, {'activity_id': actid, 'name': ' ', 'jenis': 'plan', 'partner_id': x.partner_id.id, 'location': x.location,'plan_id':plan_rels[a]})
			else:
				# PLAN
				getId = self.pool.get(tabel).create(cr, uid, {'activity_id': actid, 'name': x.name, 'partner_id': x.partner_id.id, 'location': x.location})
				plan_ids.append(getId)
			a+=1;
		return plan_ids
...
class before_actual_senin(osv.osv):
	...
	_columns = {
		...
		'location': fields.char('Location', size=64, required=True),
		'activity_id': fields.many2one('sales.activity', 'Sales Activity', required=True, ondelete='cascade'),
		'plan_id':fields.many2one('before.plan.senin',string='Plan Id',required=False,ondelete='CASCADE'),
	}
...
class after_actual_senin(osv.osv):
	...
	_columns = {
		...
		'location': fields.char('Location', size=64, required=True),
		'activity_id': fields.many2one('sales.activity', 'Sales Activity', required=True, ondelete='cascade'),
		'plan_id':fields.many2one('after.plan.senin',string='Plan Id',required=False,ondelete='CASCADE'),
	}
...
class before_actual_selasa(osv.osv):
	...
	_columns = {
		...
		'location': fields.char('Location', size=64, required=True),
		'activity_id': fields.many2one('sales.activity', 'Sales Activity', required=True, ondelete='cascade'),
		'plan_id':fields.many2one('before.plan.selasa',string='Plan Id',required=False,ondelete='CASCADE'),
	}
...
class after_actual_selasa(osv.osv):
	...
	_columns = {
		...
		'location': fields.char('Location', size=64, required=True),
		'activity_id': fields.many2one('sales.activity', 'Sales Activity', required=True, ondelete='cascade'),
		'plan_id':fields.many2one('after.plan.selasa',string='Plan Id',required=False,ondelete='CASCADE'),
	}
...
class before_actual_rabu(osv.osv):
	...
	_columns = {
		...
		'location': fields.char('Location', size=64, required=True),
		'activity_id': fields.many2one('sales.activity', 'Sales Activity', required=True, ondelete='cascade'),
		'plan_id':fields.many2one('before.plan.rabu',string='Plan Id',required=False,ondelete='CASCADE'),
	}
...
class after_actual_rabu(osv.osv):
	...
	_columns = {
		...
		'location': fields.char('Location', size=64, required=True),
		'activity_id': fields.many2one('sales.activity', 'Sales Activity', required=True, ondelete='cascade'),
		'plan_id':fields.many2one('after.plan.rabu',string='Plan Id',required=False,ondelete='CASCADE'),
	}
...
class before_actual_kamis(osv.osv):
	...
	_columns = {
		...
		'location': fields.char('Location', size=64, required=True),
		'activity_id': fields.many2one('sales.activity', 'Sales Activity', required=True, ondelete='cascade'),
		'plan_id':fields.many2one('before.plan.kamis',string='Plan Id',required=False,ondelete='CASCADE'),
	}
...
class after_actual_kamis(osv.osv):
	...
	_columns = {
		...
		'location': fields.char('Location', size=64, required=True),
		'activity_id': fields.many2one('sales.activity', 'Sales Activity', required=True, ondelete='cascade'),
		'plan_id':fields.many2one('after.plan.kamis',string='Plan Id',required=False,ondelete='CASCADE'),
	}
...
class before_actual_jumat(osv.osv):
	...
	_columns = {
		...
		'location': fields.char('Location', size=64, required=True),
		'activity_id': fields.many2one('sales.activity', 'Sales Activity', required=True, ondelete='cascade'),
		'plan_id':fields.many2one('before.plan.jumat',string='Plan Id',required=False,ondelete='CASCADE'),
	}
...
class after_actual_jumat(osv.osv):
	...
	_columns = {
		...
		'location': fields.char('Location', size=64, required=True),
		'activity_id': fields.many2one('sales.activity', 'Sales Activity', required=True, ondelete='cascade'),
		'plan_id':fields.many2one('after.plan.jumat',string='Plan Id',required=False,ondelete='CASCADE'),
	}
...
class before_actual_sabtu(osv.osv):
	...
	_columns = {
		...
		'location': fields.char('Location', size=64, required=True),
		'activity_id': fields.many2one('sales.activity', 'Sales Activity', required=True, ondelete='cascade'),
		'plan_id':fields.many2one('before.plan.sabtu',string='Plan Id',required=False,ondelete='CASCADE'),
	}
...
class after_actual_sabtu(osv.osv):
	...
	_columns = {
		...
		'location': fields.char('Location', size=64, required=True),
		'activity_id': fields.many2one('sales.activity', 'Sales Activity', required=True, ondelete='cascade'),
		'plan_id':fields.many2one('after.plan.sabtu',string='Plan Id',required=False,ondelete='CASCADE'),
	}
...
class before_actual_ahad(osv.osv):
	...
	_columns = {
		...
		'location': fields.char('Location', size=64, required=True),
		'activity_id': fields.many2one('sales.activity', 'Sales Activity', required=True, ondelete='cascade'),
		'plan_id':fields.many2one('before.plan.ahad',string='Plan Id',required=False,ondelete='CASCADE'),
	}
...
class after_actual_ahad(osv.osv):
	...
	_columns = {
		...
		'location': fields.char('Location', size=64, required=True),
		'activity_id': fields.many2one('sales.activity', 'Sales Activity', required=True, ondelete='cascade'),
		'plan_id':fields.many2one('after.plan.ahad',string='Plan Id',required=False,ondelete='CASCADE'),
	}
