...
class report_xls(report_sxw.report_sxw):
    ...
    def create(self, cr, uid, ids, data, context=None):
        ...
        report_xml_ids2 = ir_obj.search(cr, uid,
                [('report_name', '=', self.name[7:len(self.name)-4])], context=context)
        print "report_xml_ids------------------->",report_xml_ids
        if report_xml_ids:
            report_xml = ir_obj.browse(cr, uid, report_xml_ids[0], context=context)
        elif report_xml_ids2:
            report_xml = ir_obj.browse(cr, uid, report_xml_ids2[0], context=context)
        else:
            title = ''
...
#         print '==================================',locals()
        fnct_ret = fnct(cr, uid, ids or report_xml_ids2, data, report_xml, context)
        if not fnct_ret:
            return (False,False)
        return fnct_ret

    def create_source_xls(self, cr, uid, ids, data, report_xml, context=None):
        print("START:------------------------------- "+time.strftime("%Y-%m-%d %H:%M:%S")),self.name2
